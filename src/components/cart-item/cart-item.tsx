...
function CartItem({ cartItem }: CaryItemType) {
  ...
  const { name, price, vendorCode, stringCount, previewImg, id } = item; // инфа товара для отображения
  ...
  return (
    <div className="cart-item">
      <button className="cart-item__close-button button-cross" type="button" aria-label="Удалить" onClick={removeItemFromCartHandler}>
      ...
      <div className="product-info cart-item__info">
        <p className="product-info__title">ЭлектроГитара {name}</p>
      ...
      <div className="cart-item__price-total">{price * itemAmount} ₽</div>
    </div>
  );
}
